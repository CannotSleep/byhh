...
namespace Learun.Application.Organization
{
    ...
    public class UserService : RepositoryFactory
    {
        ...
        /// <param name="keyValue">主键值</param>
        /// <param name="password">新密码（MD5 小写）</param>
        public int ReviseVipPassword(string keyValue, string password)
        {
            try
            {
                //UserEntity userEntity = new UserEntity();
                //userEntity.Modify(keyValue);
                //userEntity.F_Secretkey = Md5Helper.Encrypt(CommonHelper.CreateNo(), 16).ToLower();
                //userEntity.F_Password = Md5Helper.Encrypt(DESEncrypt.Encrypt(password, "").ToLower(), 32).ToLower();

                // 会员修改密码
                var strSql = new StringBuilder();
                strSql.Append("update ");
                strSql.Append("member");
                strSql.Append(" set Password = '"+ Md5Helper.Encrypt(DESEncrypt.Encrypt(password, "").ToLower(), 32).ToLower() + "'  WHERE Id ='"+ keyValue + "'");
                return this.BaseRepository("imuStand").ExecuteBySql(strSql.ToString());
            }
            catch (Exception ex)
            {
                if (ex is ExceptionEx)
