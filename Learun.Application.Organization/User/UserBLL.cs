...
namespace Learun.Application.Organization
{
    ...
    public class UserBLL : UserIBLL
    {
        ...
        /// <param name="newPassword">新密码（MD5 小写）</param>
        /// <param name="oldPassword">旧密码（MD5 小写）</param>
        public bool ReviseVipPassword(string newPassword, string oldPassword)
        {
            try
            {
                UserInfo userInfo = LoginUserInfo.Get();
                cache.Remove("by_vip_user_Id_" + userInfo.userId, CacheId.user);
                cache.Remove("by_vip_user_account_" + userInfo.account, CacheId.user);
                //Md5Helper.Encrypt(DESEncrypt.Encrypt(password.ToLower(), "").ToLower(), 32).ToLower();
                string oldPasswordByEncrypt = Md5Helper.Encrypt(DESEncrypt.Encrypt(oldPassword.ToLower(), "").ToLower(), 32).ToLower();
                if (oldPasswordByEncrypt == userInfo.password)
                {
                    userService.ReviseVipPassword(userInfo.userId, newPassword);
                }
                else
                {
                    return false;
