...
namespace Learun.Util.Operat
{
    ...
    public class OperatorHelper
    {
        ...
        /// <param name="loginMark">登录设备标识</param>
        /// <returns>-1未登录,1登录成功,0登录过期</returns>
        public OperatorResult IsVipOnLine(string token, string loginMark)
        {
            OperatorResult operatorResult = new OperatorResult();
            operatorResult.stateCode = -1; // -1未登录,1登录成功,0登录过期
            try
            {
                ...
                if (operatorInfo != null && operatorInfo.token == token)
                {
                    ...
                    //{
                        ...
                        userInfo.token = operatorInfo.token;
                        userInfo.account = operatorInfo.account;
                        memberEntity member = vipMember.GetEntityByAccount(operatorInfo.account);
                        if (member != null)
                        {
                            ...
                            userInfo.secretkey = "";
                            userInfo.realName = member.AccountName;
                            userInfo.nickName = member.UserName;
                            userInfo.headIcon = "";
                            userInfo.gender = 0;
                            userInfo.mobile = member.MemberTel;
                            userInfo.telephone = member.Tel;
                            ...
                            userInfo.openId = 0;
                            userInfo.isSystem = 0 == 1 ? true : false;
                            userInfo.roleIds = "";
                            userInfo.postIds = "";
                            userInfo.companyIds = null;
                            userInfo.departmentIds = null;
                            userInfo.tatolPoint = member.TatolPoint.ToString();
                            userInfo.vipLevel = "";
                            userInfo.Discount = "";
                            userInfo.tatolDeposit = member.TatolDeposit.ToString();
                            if (HttpContext.Current != null)
                            {
                                HttpContext.Current.Items.Add("LoginUserInfo", userInfo);
                            }
