using Learun.Util;
using System.Data;
using System.Collections.Generic;

namespace Learun.Application.TwoDevelopment.LR_CodeDemo
{
    /// <summary>
    /// 创 建：超级管理员
    /// 日 期：2020-04-09 16:25
    /// 描 述：订单管理
    /// </summary>
    public interface OrderIBLL
    {
        #region 获取数据

        /// <summary>
        /// 获取列表数据
        /// <summary>
        /// <returns></returns>
        IEnumerable<ordersEntity> GetList( string queryJson );
        /// <summary>
        /// 获取列表分页数据
        /// <param name="pagination">分页参数</param>
        /// <summary>
        /// <returns></returns>
        IEnumerable<ordersEntity> GetPageList(Pagination pagination, string queryJson);
        /// <summary>
        /// 获取实体数据
        /// <param name="keyValue">主键</param>
        /// <summary>
        /// <returns></returns>
        ordersEntity GetEntity(string keyValue);
        /// <summary>
        /// 根据用户ID获取订单数据
        /// <param name="keyValue">主键</param>
        /// <summary>
        /// <returns></returns>
        IEnumerable<ordersEntity> GetByUserId(string orderid, Pagination pagination);
        /// <summary>
        /// 根据ID获取订单数据
        /// <param name="keyValue">主键</param>
        /// <summary>
        /// <returns></returns>
        IEnumerable<ordersEntity> GetById(string id);
        #endregion

        #region 提交数据

        /// <summary>
        /// 删除实体数据
        /// <param name="keyValue">主键</param>
        /// <summary>
        /// <returns></returns>
        void DeleteEntity(string keyValue);
        /// <summary>
        /// 保存实体数据（新增、修改）
        /// <param name="keyValue">主键</param>
        /// <summary>
        /// <returns></returns>
        void SaveEntity(string keyValue, ordersEntity entity);
        #endregion

    }
}
