...
namespace Learun.Application.WebApi
{
    ...
    public class BaseApi : NancyModule
    {
        ...
        private Response BeforeRequest(NancyContext ctx)
        {
            ...
            token = req.token;  
            if (path == "/by/adms/user/login" || path == "/" || path == "/bgimg" || path == "/by/adms/user/img" || path == "/by/adms/desktop/img" || path == "/by/search/ys" || path == "/by/standard/getById" || path == "/by/adms/user/verify" || path == "/by/adms/user/register" || path == "/by/news/getlist" || path == "/by/news/getitem" || path == "/by/mechanism/getlist" || path == "/by/standard/getStandardCount" || path == "/by/standard/gethotword" || path == "/by/search/gs" || path == "/by/standard/getrecentlyreleased" || path == "/by/standard/getcomingsoon" || path == "/by/standard/down" || path == "/by/standard/preview")
            {// 登录接口，默认页面接口不做权限验证处理
                return null;
            }

            OperatorResult res = OperatorHelper.Instance.IsVipOnLine(req.token, req.loginMark);
            if (res.stateCode == -1)
            {
